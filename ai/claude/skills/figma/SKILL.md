---
name: figma
description: Fetch designs from Figma, generate React/React Native code, sync design tokens, and gather Figma context for ticket intake pipeline. Use when the user shares a Figma URL, asks to implement a design, wants to extract design tokens, or when the jira inbox workflow finds Figma links.
...
When invoked as part of the jira inbox pipeline (not directly by user):

This skill can be called by the jira skill's fetch workflow when Figma links are found in a Jira ticket. In this mode, the goal is NOT code generation — it's context gathering.

**Input:** Figma URL(s) extracted from ticket description/comments
**Output:** Design data saved to `todo/MT-XXXX/context/` directory

See `rules/integration.md` for detailed implementation.

### What gets saved:
- `todo/MT-XXXX/context/figma-{FILE_KEY}.json` — Structured design data (component tree, properties, styles)
- `todo/MT-XXXX/context/figma-{FILE_KEY}-screenshot.png` — Visual screenshot of the referenced node/frame

### When this differs from direct invocation:
- Direct (`/figma`): Interactive — asks user what to do (view specs, generate code, sync tokens)
- Pipeline (`rules/integration.md`): Automated — fetches and saves without user interaction

## Rules

Detailed implementation patterns are in the `rules/` subdirectory:

- `rules/integration.md` — Figma context gathering for jira ticket intake pipeline
